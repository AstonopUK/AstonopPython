...
worldmap = [["Castle Holdent","Wide Orchard","Rolling Fields","Urbos Hamlet","Western Badlands"],
            ["Grassy Grove","Rushly Village","Swampy Knoll","Kwarmi Jungle","The Expanse"],
            ["Arid Lowlands","Sand Banks","Vast Field","Pumpkin Patch","Northern Badlands"],
            ["Low-lying Mires","Waterlogged Village","Riverside Cliffs","High Peaks","The Pitch Black"],
            ["Hazey Frostlands","Frozen Lake","Snowy Town","Desolate Hills","Eastern Badlands"]]
            ...
            case "up":
                ...
                if ((random.randint(0,30)*(((playerChar.xCoord + 1) + (playerChar.yCoord + 1))/2)) > 35):
                    for x in range(random.randint(0,2)):
                        enemies.append(enemy(random.randint(1,(3+int(playerChar.level/2))), random.randint(5,10*(playerChar.level/2)), enemyNames[random.randint(0, len(enemyNames)-1)], random.randint(20,50*playerChar.level), loot))
            case "down":
                playerChar.xCoord -= 1
                ...
                if ((random.randint(0,30)*(((playerChar.xCoord + 1) + (playerChar.yCoord + 1))/2)) > 35):
                    for x in range(random.randint(0,2)):
                        enemies.append(enemy(random.randint(1,(3+int(playerChar.level/2))), random.randint(5,20+(3+int(playerChar.level/2))), enemyNames[random.randint(0, len(enemyNames)-1)], random.randint(20,50*playerChar.level), loot))
            case "right":
                playerChar.yCoord += 1
